...
- (id<OrgOcpsoftPrettytimeDuration>)approximateDurationWithJavaUtilDate:(JavaUtilDate *)then {
  ...
  long long int difference = [((JavaUtilDate *) nil_chk(then)) getTime] - [((JavaUtilDate *) nil_chk(ref)) getTime];
...
- (id<OrgOcpsoftPrettytimeDuration>)calculateDurationWithLong:(long long int)difference {
  long long int absoluteDifference = [JavaLangMath absWithLong:difference];
  ...
  for (int i = 0; i < [((id<JavaUtilList>) nil_chk(localUnits)) size]; i++) {
    id<OrgOcpsoftPrettytimeTimeUnit> unit = [localUnits getWithInt:i];
    long long int millisPerUnit = [JavaLangMath absWithLong:[((id<OrgOcpsoftPrettytimeTimeUnit>) nil_chk(unit)) getMillisPerUnit]];
    long long int quantity = [JavaLangMath absWithLong:[unit getMaxQuantity]];
    BOOL isLastUnit = (i == [localUnits size] - 1);
...
- (id<JavaUtilList>)calculatePreciseDurationWithJavaUtilDate:(JavaUtilDate *)then {
  ...
  long long int difference = [((JavaUtilDate *) nil_chk(then)) getTime] - [((JavaUtilDate *) nil_chk(reference_)) getTime];
...
- (NSString *)formatWithJavaUtilList:(id<JavaUtilList>)durations {
  ...
  if (durations != nil) {
    ...
    for (int i = 0; i < [durations size]; i++) {
      ...
      BOOL isLast = (i == [durations size] - 1);
      if (!isLast) {
...
- (id<JavaUtilList>)getUnits {
  if (mCachedUnits_ == nil) {
    ...
    [JavaUtilCollections sortWithJavaUtilList:result withJavaUtilComparator:[[OrgOcpsoftPrettytimeUnitsTimeUnitComparator alloc] init]];
    mCachedUnits_ = [JavaUtilCollections unmodifiableListWithJavaUtilList:result];
  }
...
- (id)getUnitWithIOSClass:(IOSClass *)unitType {
  ...
  for (id<OrgOcpsoftPrettytimeTimeUnit> __strong unit in nil_chk([((id<JavaUtilMap>) nil_chk(units_)) keySet])) {
    if ([nil_chk(unitType) isAssignableFrom:[((id<OrgOcpsoftPrettytimeTimeUnit>) nil_chk(unit)) getClass]]) {
      return (id<OrgOcpsoftPrettytimeTimeUnit>) unit;
    }
  }
  return nil;
}
...
- (NSString *)description {
  return [NSString stringWithFormat:@"PrettyTime [reference=%@, locale=%@]", reference_, locale_];
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _OrgOcpsoftPrettytimePrettyTime = { "PrettyTime", "org.ocpsoft.prettytime", NULL, 0x1, 30, methods, 4, fields, 0, NULL};
