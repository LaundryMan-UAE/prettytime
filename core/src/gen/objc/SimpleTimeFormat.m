...
- (NSString *)applyPatternWithNSString:(NSString *)sign
  ...
  result = [((NSString *) nil_chk(result)) replaceAll:OrgOcpsoftPrettytimeFormatSimpleTimeFormat_QUANTITY_ withReplacement:[NSString valueOfLong:quantity]];
...
- (NSString *)getSingularNameWithOrgOcpsoftPrettytimeDuration:(id<OrgOcpsoftPrettytimeDuration>)duration {
  if ([((id<OrgOcpsoftPrettytimeDuration>) nil_chk(duration)) isInFuture] && futureSingularName_ != nil && ((int) [futureSingularName_ length]) > 0) return futureSingularName_;
  else if ([duration isInPast] && pastSingularName_ != nil && ((int) [pastSingularName_ length]) > 0) return pastSingularName_;
  else return singularName_;
}

- (NSString *)getPluralNameWithOrgOcpsoftPrettytimeDuration:(id<OrgOcpsoftPrettytimeDuration>)duration {
  if ([((id<OrgOcpsoftPrettytimeDuration>) nil_chk(duration)) isInFuture] && futurePluralName_ != nil && ((int) [((NSString *) nil_chk(futureSingularName_)) length]) > 0) return futurePluralName_;
  else if ([duration isInPast] && pastPluralName_ != nil && ((int) [((NSString *) nil_chk(pastSingularName_)) length]) > 0) return pastPluralName_;
...
- (NSString *)description {
  return [NSString stringWithFormat:@"SimpleTimeFormat [pattern=%@, futurePrefix=%@, futureSuffix=%@, pastPrefix=%@, pastSuffix=%@, roundingTolerance=%d]", pattern_, futurePrefix_, futureSuffix_, pastPrefix_, pastSuffix_, roundingTolerance_];
}
...
+ (J2ObjcClassInfo *)__metadata {
  ...
  static J2ObjcClassInfo _OrgOcpsoftPrettytimeFormatSimpleTimeFormat = { "SimpleTimeFormat", "org.ocpsoft.prettytime.format", NULL, 0x1, 27, methods, 16, fields, 0, NULL};
